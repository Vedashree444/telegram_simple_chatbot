...
def echo_all(updates):
    for update in updates["result"]:
        ...
        if last_chat_text.lower() in greetings and today == now.day and 6 <= hour < 12:
            ...
            text = 'hello....Good Evening {}'.format(last_chat_name)+'\nHad dinner ?'
            send_telegram_message(text, last_chat_id)
            today += 1
        if last_chat_text.lower() in abc:
            text = 'okay {}'.format(last_chat_name)
        if last_chat_text.lower() in final:
            text = 'see u\nhave a great day {}\n'.format(last_chat_name);
        if last_chat_text.lower() in gn:
            text = 'Good Night\n'.format(last_chat_name);
